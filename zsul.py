...
decoded_vars = json.loads(base64.b64decode(encoded_vars))

todos = []

for j in [49, 89]:
    for cluster in ['2364', '1131', '7350', '164', '1604', '868', '238']:
        erro = 0
        i = 0
        total = 1
        while i < total:
            decoded_vars2 = dict(decoded_vars)
            ...
            try:
                ...
                data = response.json().get('data', {}).get('productSearch', {})
                produtos = data.get('products', []) or []
                ...
                print('Erro na iteração', i, e, 'i', i, 'tam', len(produtos) if 'produtos' in dir() else 0, 'total', total, 'todos', len(todos))
                erro += 1
                if erro > 2:
...
df2 = df[['linkText', 'product_name', 'selling_price', 'measurement_unit', 'unit_multiplier', 'sku_id', 'product_id', 'brand']].copy()
df2['data'] = hoje
...
print(f'Zona Sul: {len(df2)} produtos gravados em {filename}')
